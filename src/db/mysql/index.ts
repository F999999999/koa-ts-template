...
import { PoolConnection, QueryOptions } from "mysql2";
...
// 提交事务
export const commitTransaction = async (conn: PoolConnection | null) => {
  if (conn) {
    // 开发环境中打印
    if (process.env.NODE_ENV === "development") {
...
// 回滚事务
export const rollbackTransaction = async (conn: PoolConnection | null) => {
  // 开发环境中打印
  if (process.env.NODE_ENV === "development") {
